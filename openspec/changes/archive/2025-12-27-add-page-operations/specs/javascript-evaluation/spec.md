...
- **WHEN** `page.wait_for_function("() => document.querySelector('.loaded')").await` is called
- **THEN** the method returns when the selector matches an element

#### Scenario: Wait for function with argument

- **GIVEN** a page with dynamic content
- **WHEN** `page.wait_for_function_with_arg("sel => document.querySelector(sel)", ".ready").await` is called
- **THEN** the method returns when the condition is met

#### Scenario: Wait with RAF polling (default)

- **GIVEN** a page with content
- **WHEN** `page.wait_for_function("() => window.ready").await` is called
- **THEN** the function is evaluated on each requestAnimationFrame by default

#### Scenario: Wait with interval polling

- **GIVEN** a page with content
- **WHEN** `page.wait_for_function("() => window.ready").polling(Polling::Interval(Duration::from_millis(100))).await` is called
- **THEN** the function is evaluated every 100 milliseconds

#### Scenario: Wait with custom timeout

- **GIVEN** a page with content
- **WHEN** `page.wait_for_function("() => false").timeout(Duration::from_secs(1)).await` is called
...
- **GIVEN** a page with default timeout of 30 seconds
- **WHEN** `page.wait_for_function("() => false").await` is called
- **AND** the condition never becomes true
- **THEN** a timeout error is returned after 30 seconds

#### Scenario: Wait returns JSHandle

- **GIVEN** a page with content
- **WHEN** `page.wait_for_function("() => document.body").await` is called
- **THEN** a JsHandle referencing the truthy return value is returned
