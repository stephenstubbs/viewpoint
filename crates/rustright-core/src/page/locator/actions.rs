...
use rustright_cdp::protocol::input::{
    DispatchKeyEventParams, DispatchMouseEventParams, InsertTextParams, MouseButton,
};
use rustright_cdp::protocol::runtime::EvaluateParams;
...
impl Locator<'_> {
    ...
    pub async fn fill(&self, text: &str) -> Result<(), LocatorError> {
        ...
        select_all.modifiers = Some(rustright_cdp::protocol::input::modifiers::CTRL);
    ...
    pub async fn press(&self, key: &str) -> Result<(), LocatorError> {
        ...
        for part in &parts[..parts.len().saturating_sub(1)] {
            match part.to_lowercase().as_str() {
                "control" | "ctrl" => {
                    modifiers |= rustright_cdp::protocol::input::modifiers::CTRL;
                }
                "alt" => modifiers |= rustright_cdp::protocol::input::modifiers::ALT,
                "shift" => modifiers |= rustright_cdp::protocol::input::modifiers::SHIFT,
                "meta" | "cmd" => modifiers |= rustright_cdp::protocol::input::modifiers::META,
                _ => {}
            }
        }
