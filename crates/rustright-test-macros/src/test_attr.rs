//! Implementation of the `#[rustright::test]` attribute macro.
...
fn generate_harness_setup(args: &TestArgs) -> Result<TokenStream> {
    ...
    match args.scope.as_deref() {
        Some("browser") => {
            ...
            Ok(quote! {
                ::rustright_test::TestHarness::from_browser(#browser_fn().await).await?
            })
        }
        Some("context") => {
            let context_fn: Ident = syn::parse_str(args.context.as_ref().unwrap())?;
            Ok(quote! {
                ::rustright_test::TestHarness::from_context(#context_fn().await).await?
            })
        }
        None => {
            Ok(quote! {
                ::rustright_test::TestHarness::builder()
                    .headless(#headless)
