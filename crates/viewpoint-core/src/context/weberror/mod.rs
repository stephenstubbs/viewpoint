...
pub type WebErrorHandler =
    Box<dyn Fn(WebError) -> Pin<Box<dyn Future<Output = ()> + Send>> + Send + Sync>;
...
impl BrowserContext {
            ...
            while let Ok(event) = events.recv().await {
                if event.method == "Runtime.exceptionThrown" {
                    if let Some(params) = &event.params {
                        if let Ok(exception_event) =
                            serde_json::from_value::<ExceptionThrownEvent>(params.clone())
                        {
                            ...
                            if let Some(ref h) = *handler {
                                ...
                                debug!(
                                    "Web error in context {} (no handler): {}",
                                    context_id,
                                    web_error.message()
                                );
                            }
        ...
        Fut: Future<Output = ()> + Send + 'static,
    {
        ...
        *weberror_handler = Some(Box::new(move |error| Box::pin(handler(error))));
    }
