...
async fn test_main_frame() {
    ...
    assert!(url.contains("example.com"), "Main frame URL should contain example.com, got: {}", url);
    
    // Main frame should be the main frame
    assert!(main_frame.is_main(), "Should be main frame");
    assert!(main_frame.parent_id().is_none(), "Main frame should have no parent");
...
async fn test_list_frames() {
    ...
    assert!(frames.len() >= 3, "Should have at least 3 frames (main + 2 iframes), got {}", frames.len());
...
async fn test_frame_properties() {
    ...
    assert!(content.contains("Example Domain"), "Content should contain page text");
    ...
    // Test is_detached
    assert!(!main_frame.is_detached(), "Main frame should not be detached");
...
async fn test_frame_attached_event() {
    ...
    assert!(handler_called.load(Ordering::SeqCst), "Frame attached handler should have been called");
...
async fn test_frame_detached_event() {
    ...
    // Handler should have been called
    assert!(handler_called.load(Ordering::SeqCst), "Frame detached handler should have been called");
