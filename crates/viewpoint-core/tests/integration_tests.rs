//! Integration tests for viewpoint-core.
//!
...
async fn test_locator_creation() {
    ...
    let by_role = page.get_by_role(viewpoint_core::AriaRole::Heading).build();

    // Verify they have the expected selectors
    assert!(matches!(by_css.selector(), viewpoint_core::Selector::Css(_)));
    assert!(matches!(by_text.selector(), viewpoint_core::Selector::Text { .. }));
    assert!(matches!(by_role.selector(), viewpoint_core::Selector::Role { .. }));
...
async fn test_locator_chaining() {
    ...
    assert!(matches!(chained.selector(), viewpoint_core::Selector::Chained(_, _)));
...
async fn test_locator_nth_selection() {
    ...
    assert!(matches!(first.selector(), viewpoint_core::Selector::Nth { .. }));
    assert!(matches!(last.selector(), viewpoint_core::Selector::Nth { .. }));
    assert!(matches!(nth.selector(), viewpoint_core::Selector::Nth { .. }));
...
async fn test_get_by_label() {
    ...
    assert!(matches!(label_locator.selector(), viewpoint_core::Selector::Label { .. }));
...
async fn test_get_by_placeholder() {
    ...
    assert!(matches!(placeholder_locator.selector(), viewpoint_core::Selector::Placeholder { .. }));
...
async fn test_get_by_test_id() {
    ...
    assert!(matches!(test_id_locator.selector(), viewpoint_core::Selector::TestId(_)));
