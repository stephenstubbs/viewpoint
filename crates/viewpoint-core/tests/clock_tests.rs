...
async fn test_clock_date_mocking() {
    ...
    clock.set_fixed_time("2024-01-01T00:00:00Z")
        .await
        .expect("Failed to set fixed time");
    
    // Get Date.now() and verify it's the expected timestamp
    let timestamp: f64 = page.evaluate(js!{ Date.now() }).await.expect("Failed to get time");
    ...
    let date_string: String = page.evaluate(js!{ new Date().toISOString() })
        .await
...
async fn test_clock_timer_mocking() {
    ...
    let _: f64 = page.evaluate(js!{ setTimeout(() => { window.timerFired = true; }, 5000) })
        .await
        .expect("Failed to set timer");
    
    // Timer should not have fired yet
    let fired_before: bool = page.evaluate(js!{ window.timerFired })
        .await
        .expect("Failed to check timer");
    assert!(!fired_before, "Timer should not have fired yet");
    
    // Advance time by 5 seconds
    let timers_fired = clock.run_for(Duration::from_secs(5))
        .await
    ...
    let fired_after: bool = page.evaluate(js!{ window.timerFired })
        .await
        .expect("Failed to check timer");
...
async fn test_clock_fast_forward() {
    ...
    let mut clock = page.clock();
    clock.install().await.expect("Failed to install clock");
    clock.set_fixed_time(1704067200000i64).await.expect("Failed to set time");
    
    // Get initial time
    let time_before: f64 = page.evaluate(js!{ Date.now() }).await.expect("Failed to get time");
    ...
    let time_after: f64 = page.evaluate(js!{ Date.now() }).await.expect("Failed to get time");
    let expected_time = time_before + 3600.0 * 1000.0;
    assert!((time_after - expected_time).abs() < 1000.0, 
        "Time should have advanced by ~1 hour. Before: {}, After: {}", time_before, time_after);
...
async fn test_clock_run_all_timers() {
    ...
    let _: serde_json::Value = page.evaluate(js!{
    ...
    }).await.expect("Failed to set timers");
    
    // Check pending timer count
    let pending = clock.pending_timer_count().await.expect("Failed to get count");
    ...
    // Count should be 3
    let count: f64 = page.evaluate(js!{ window.count }).await.expect("Failed to get count");
...
async fn test_clock_system_time() {
    ...
    clock.set_system_time("2024-06-15T12:00:00Z")
        .await
        .expect("Failed to set system time");
    
    // Get initial time
    let time1: f64 = page.evaluate(js!{ Date.now() }).await.expect("Failed to get time");
    
    // Wait a bit (real time)
    tokio::time::sleep(Duration::from_millis(100)).await;
    
    // Get time again - it should have advanced with real time
    let time2: f64 = page.evaluate(js!{ Date.now() }).await.expect("Failed to get time");
    
    // Time should have advanced (allowing some tolerance)
    assert!(time2 > time1, "System time should flow: {} should be > {}", time2, time1);
...
async fn test_clock_pause_resume() {
    ...
    clock.pause_at("2024-01-01T12:00:00Z")
        .await
    ...
    let time3: f64 = page.evaluate(js!{ Date.now() }).await.expect("Failed to get time");
