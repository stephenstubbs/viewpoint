...
async fn test_dialog_prompt_accept_with_text() {
    ...
    page.on_dialog(|dialog| async move { dialog.accept_with_text("Custom Answer").await })
        .await;
    ...
    assert_eq!(
        ...
        "Prompt should return custom text when accepted"
    );
...
async fn test_dialog_beforeunload_event() {
    ...
    page.set_content(BEFOREUNLOAD_HTML)
        .set()
        .await
        .expect("Failed to set content");
    ...
    if received_type.is_some() {
        assert!(
            matches!(received_type, Some(DialogType::Beforeunload)),
...
async fn test_dialog_off_dialog() {
    ...
    assert_eq!(
        *handler_called_count.lock().await,
        1,
        "Handler should not be called after off_dialog"
    );
