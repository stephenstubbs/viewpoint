...
async fn test_js_evaluate_primitives() {
    ...
    // Test integer
    let num: i32 = page.evaluate(js!{ 1 + 2 }).await.expect("Failed to evaluate int");
    assert_eq!(num, 3);
    
    // Test float
    let float: f64 = page.evaluate(js!{ 3.14 * 2 }).await.expect("Failed to evaluate float");
    assert!((float - 6.28).abs() < 0.01);
    
    // Test boolean
    let boolean: bool = page.evaluate(js!{ true && false }).await.expect("Failed to evaluate bool");
    assert!(!boolean);
    
    // Test string
    let string: String = page.evaluate(js!{ "hello" + " " + "world" }).await.expect("Failed to evaluate string");
    assert_eq!(string, "hello world");
    
    // Test null (returns Option)
    let null_val: Option<String> = page.evaluate(js!{ null }).await.expect("Failed to evaluate null");
...
async fn test_js_evaluate_objects() {
    ...
    let obj: serde_json::Value = page.evaluate(js!{ ({ a: 1, b: "two", c: true }) })
        .await
    ...
    // Test array
    let arr: Vec<i32> = page.evaluate(js!{ [1, 2, 3, 4, 5] })
        .await
...
async fn test_js_evaluate_promises() {
    ...
    let result: String = page.evaluate(js!{
        Promise.resolve("resolved value")
    }).await.expect("Failed to evaluate promise");
    assert_eq!(result, "resolved value");
    
    // Test async function
    let async_result: i32 = page.evaluate(js!{
        (async () => {
            await new Promise(r => setTimeout(r, 10));
            return 42;
        })()
    }).await.expect("Failed to evaluate async function");
...
async fn test_js_interpolation_types() {
    ...
    let num = 42;
    let result: i32 = page.evaluate(&js!{ #{num} * 2 }).await.expect("Failed to evaluate");
    assert_eq!(result, 84);
    
    // Test float interpolation
    let pi = 3.14159;
    let result: f64 = page.evaluate(&js!{ Math.round(#{pi} * 100) / 100 }).await.expect("Failed to evaluate");
    assert!((result - 3.14).abs() < 0.01);
    
    // Test string interpolation
    let name = "World";
    let result: String = page.evaluate(&js!{ "Hello, " + #{name} + "!" }).await.expect("Failed to evaluate");
    assert_eq!(result, "Hello, World!");
    
    // Test string with special characters
    let special = "it's a \"test\"\nwith newlines";
    let result: String = page.evaluate(&js!{ #{special} }).await.expect("Failed to evaluate");
    assert_eq!(result, special);
    
    // Test boolean interpolation
    let flag = true;
    let result: bool = page.evaluate(&js!{ #{flag} && true }).await.expect("Failed to evaluate");
    assert!(result);
    
    // Test multiple interpolations
    let x = 10;
    let y = 20;
    let result: i32 = page.evaluate(&js!{ #{x} + #{y} }).await.expect("Failed to evaluate");
    assert_eq!(result, 30);
...
async fn test_js_interpolation_function_args() {
    ...
    let arr = "[1, 2, 3, 4, 5]";
    let _: () = page.evaluate(&js!{ void(window.testArray = JSON.parse(#{arr})) })
        .await
        .expect("Failed to evaluate");
    
    // Verify the array was set correctly
    let length: i32 = page.evaluate(js!{ window.testArray.length }).await.expect("Failed to get length");
    assert_eq!(length, 5);
    
    // Test interpolation with object property access
    let key = "testKey";
    let value = "testValue";
    let _: () = page.evaluate(&js!{ void(window[#{key}] = #{value}) })
        .await
        .expect("Failed to evaluate");
    
    let retrieved: String = page.evaluate(&js!{ window[#{key}] })
        .await
...
async fn test_js_interpolation_complex() {
    ...
    let result: i32 = page.evaluate(&js!{ #{computed} }).await.expect("Failed to evaluate");
    assert_eq!(result, 500);
    
    // Test with String vs &str
    let owned_string = String::from("owned");
    let result: String = page.evaluate(&js!{ #{owned_string} })
        .await
        .expect("Failed to evaluate");
...
async fn test_js_evaluate_with_interpolation() {
    ...
    let multiplier = 5;
    let result: i32 = page.evaluate(&js!{ 10 * #{multiplier} })
        .await
    ...
    let selector = "#test";
    let text: String = page.evaluate(&js!{ document.querySelector(#{selector}).textContent })
        .await
    ...
    let a = 10;
    let b = 20;
    let sum: i32 = page.evaluate(&js!{ #{a} + #{b} })
        .await
